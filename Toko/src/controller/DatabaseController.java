...
public class DatabaseController extends Myconfig {
    ...
    public static List<Product> getDatabase() {
        ...
        try {
            ...
            query = "SELECT ID, NAMA, HARGA, STOK FROM table_product";
            ...
            while(resultSet.next()) {
                ...
                Product produk = new Product(id, nama, harga, jumlah);
...
//    }
    ...
    public static void updateNamaDB(int id, String name) {
        connection();
        query= "UPDATE `table_product` SET NAMA=? WHERE ID=?";
        try {
            preparedStatement = connect.prepareStatement(query);
    ...
    public static boolean deletDB(int ID) {
        connection();
        query= "DELETE FROM `table_product` WHERE ID=?";
        try {
            preparedStatement = connect.prepareStatement(query);
